...
class TestPlantCardCooldown:
    """测试植物卡片冷却管理"""

    def test_plant_card_manager_initialization(self):
        """测试卡片管理器初始化"""
        ...
        assert len(manager.card_cooldowns) == 0

    def test_register_plant_card(self):
        """测试注册植物卡片"""
        from src.systems.plant_card_manager import PlantCardManager
        
        manager = PlantCardManager()
        manager.register_card("PEASHOOTER", cooldown_time=5.0)
        
        assert "PEASHOOTER" in manager.card_cooldowns
        assert manager.card_cooldowns["PEASHOOTER"] == 5.0

    def test_can_plant_when_ready(self):
        """测试冷却完成时可以种植"""
        from src.systems.plant_card_manager import PlantCardManager
        ...
        assert manager.can_plant("PEASHOOTER") is True

    def test_cannot_plant_during_cooldown(self):
        """测试冷却期间不能种植"""
        from src.systems.plant_card_manager import PlantCardManager
        ...
        # 开始种植（触发冷却）
        manager.start_cooldown("PEASHOOTER")
        
        assert manager.can_plant("PEASHOOTER") is False

    def test_cooldown_progress(self):
        """测试冷却进度"""
        from src.systems.plant_card_manager import PlantCardManager
        ...
        progress = manager.get_cooldown_progress("PEASHOOTER")
        assert progress == 0.4  # 2/5 = 0.4

    def test_cooldown_complete_after_time(self):
        """测试冷却时间结束后可以再次种植"""
        from src.systems.plant_card_manager import PlantCardManager
        ...
        assert manager.can_plant("PEASHOOTER") is True
        assert manager.get_cooldown_progress("PEASHOOTER") == 1.0
    ...
    def test_multiple_cards_independent(self):
        ...
        manager = PlantCardManager()
        manager.register_card("PEASHOOTER", cooldown_time=5.0)
        manager.register_card("SUNFLOWER", cooldown_time=3.0)
        
        # 只种植豌豆射手
        manager.start_cooldown("PEASHOOTER")
        
        assert manager.can_plant("PEASHOOTER") is False
        assert manager.can_plant("SUNFLOWER") is True
    ...
    def test_unknown_plant_type(self):
        ...
        assert manager.can_plant("UNKNOWN") is False
        assert manager.get_cooldown_progress("UNKNOWN") == 0.0
...
class TestCooldownIntegration:
    ...
    def test_cooldown_visual_feedback(self):
        """测试冷却视觉反馈"""
        from src.systems.plant_card_manager import PlantCardManager
        ...
        # 开始冷却
        manager.start_cooldown("PEASHOOTER")
        assert manager.get_cooldown_progress("PEASHOOTER") == 0.0
        
        # 更新2.5秒
        manager.update(2.5)
        assert manager.get_cooldown_progress("PEASHOOTER") == 0.5
